...
// 树查询方法，根据属性和值查询节点
export function findTreeNodeByProperty(tree, property, value,childrenName = 'children') {
  // 如果树是一个数组（多棵树的集合）
  if (Array.isArray(tree)) {
    for (let node of tree) {
      const foundNode = findTreeNodeByProperty(node, property, value);
      if (foundNode) {
        return foundNode;
      }
    }
  ...
  if (tree[childrenName] && tree[childrenName].length > 0) {
    for (let child of tree[childrenName]) {
      const foundNode = findTreeNodeByProperty(child, property, value);
      if (foundNode) {
        return foundNode;
      }
    }
